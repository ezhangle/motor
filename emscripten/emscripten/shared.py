...
# On Windows python2 suffers from a particularly nasty bug if python2 is spawning new processes while python2 itself is spawned from some other non-console process.
...
# See http://bugs.python2.org/issue3905
class WindowsPopen:
...
# Install our replacement Popen handler if we are running on Windows to avoid python2 spawn process function.
...
if EM_POPEN_WORKAROUND and os.name == 'nt':
  logging.debug('Installing Popen workaround handler to avoid bug http://bugs.python2.org/issue3905')
  Popen = WindowsPopen
...
# If we have 'env', we should use that to find python2, because |python2| may fail while |env python2| may work
# (For example, if system python2 is 3.x while we need 2.x, and env gives 2.x if told to do so.)
ENV_PREFIX = []
if not WINDOWS:
  try:
...
class Settings2(type):
  class __impl:
    ...
    @classmethod
    def load(self, args=[]):
      # Load the JS defaults into python2
    ...
    def serialize(self):
      ...
      for key, value in self.attrs.iteritems():
        if key == key.upper(): # this is a hack. all of our settings are ALL_CAPS, python2 internals are not
...
class Building:
  ...
  def get_building_env(native=False):
    ...
    if native:
      ...
      return env
    env['CC'] = EMCC if not WINDOWS else 'python2 %r' % EMCC
    env['CXX'] = EMXX if not WINDOWS else 'python2 %r' % EMXX
    env['AR'] = EMAR if not WINDOWS else 'python2 %r' % EMAR
    env['LD'] = EMCC if not WINDOWS else 'python2 %r' % EMCC
    env['LDSHARED'] = EMCC if not WINDOWS else 'python2 %r' % EMCC
    env['RANLIB'] = EMRANLIB if not WINDOWS else 'python2 %r' % EMRANLIB
